...
function npConfirm() {
    ...
    _closeNumpadCleanup();
    // SP版カートも更新
    if (typeof spRenderCart === 'function' && isSP()) spRenderCart();
    ...
    if (spType === 'win-place') {
        ...
        container.innerHTML = list.map(item => {
            ...
            return `<div class="sp-odds-item" style="cursor:default;">
                ...
                <div style="display:flex;gap:6px;padding:0 8px;align-items:center;flex-shrink:0;">
                    <div style="text-align:center;cursor:pointer;padding:6px 0;border:1.5px solid #ddd;border-radius:6px;background:#fff;width:64px;" onclick="spAddBet('win','${item.no}','単勝')">
                        <div class="sp-odds-val ${getOddsClass(item.win,'win')}">${item.win||'---'}</div>
                        <div style="font-size:.62rem;color:#aaa;">単勝</div>
                    </div>
                    <div style="text-align:center;cursor:pointer;padding:6px 0;border:1.5px solid #ddd;border-radius:6px;background:#fff;width:64px;" onclick="spAddBet('place','${item.no}','複勝')">
...
async function spSetMs() {
    ...
    const newTotal = cart.reduce((s,i) => s + i.combs.length * i.amountPerBet, 0) + combs.length * 100;
    if (newTotal > BUDGET_LIMIT) {
        await psAlert(`予算超過：合計が${BUDGET_LIMIT.toLocaleString()}ptを超えるため追加できません。\n（この買い目は${combs.length}点 = ${(combs.length*100).toLocaleString()}pt）`);
        return;
    }
...
async function spAddBet(type, formation, label) {
    const newTotal = cart.reduce((s,i) => s + i.combs.length * i.amountPerBet, 0) + 100;
    if (newTotal > BUDGET_LIMIT) {
        await psAlert(`予算超過：合計が${BUDGET_LIMIT.toLocaleString()}ptを超えるため追加できません。`);
        return;
    }
...
function spToggleCart() {
    const wrap = document.getElementById('sp-cart-wrap');
    // minimized状態のときはminimizedを解除するだけ
    if (wrap.classList.contains('minimized')) {
        wrap.classList.remove('minimized');
        document.getElementById('sp-minimize-btn').textContent = '最小化';
        return;
    }
